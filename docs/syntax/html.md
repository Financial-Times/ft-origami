...
* Conditional comments *must not* be used in components or recommended to product developers. Components should instead rely on classes on the `body` or `html` element that indicate feature support. Product developers may of course choose to apply those classes using conditional comments.
