...
Where a module contains both markup and JavaScript, it *should* advise the use of a `<noscript>` tag to offer feature fallback to users where the product developer opts not to run the JavaScript, or the user agent does not support it.  Advising the use of the `<noscript>` *should* be done via a Mustache template.

When providing `<noscript>` tag examples, module developers *must* include the module name and the string 'origami' as classes on the tag:

    <noscript class='origami o-comment'>To comment on this article, you need to upgrade your web browser.  <a href='...'>Learn how to upgrade</a>.</noscript>
