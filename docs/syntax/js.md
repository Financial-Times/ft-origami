...
Any configuration option expecting a `function` *must* not be defined in a declarative config block and *must* be optional, providing default behaviour where the imperative configuration using `init()` is absent.  If a configuration key is present in the declarative config block that xpects a `function`, an `Error` *should* be thrown to warn the developer of the invalid configuration.
